...
class Field:
    ...
    def get_board_int(self):
        ...
        for r in range(FIELD_SIZE[1]):
            for c in range(FIELD_SIZE[0]):
                if self.f[r][c] is None:
                    boardint[FIELD_SIZE[1] - r - 1][c] = 0
                else:
                    boardint[FIELD_SIZE[1] - r - 1][c] = 1
                # print(boardint[c][r], end="")
            # print("")
        return boardint
    ...
    def get_possible_placements(self, b: Block):
        ...
        for rot in range(4):
            ...
            for col in range(FIELD_SIZE[0] - b.curr_width() + 1):
                ...
                if(b.y <= 0): continue;
                print(b.y)
                b.field.check_filled()
                placements.append(b.field.get_board_int())
                cols.append(col)
                rots.append(rot)
                b.remove()
                b.field = copy.deepcopy(original_field)
                b.set_row(0)
...
def main():
    ...
    while over == 0:
        ...
        picked = ta.pick(placements, cols, rots)
        print(picked, " picked col:",cols[picked]," rot:", rots[picked])
        block.set_rot(rots[picked])
        block.set_col(cols[picked]-block.displacement())
